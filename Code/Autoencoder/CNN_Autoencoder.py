...
class Encoder(nn.Module):
    ...
    def __init__(self, encoded_space_dim):
        ...
        self.encoder_cnn = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=16, kernel_size=2,  stride=0, padding=0), # 8 x 8
            nn.ReLU(True),
            nn.BatchNorm2d(16),
            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=4,  stride=2, padding=2),# 4 x 4
            nn.ReLU(True),
            nn.BatchNorm2d(32),
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=2,  stride=1, padding=0), # 2 x 2
            nn.ReLU(True),
            nn.BatchNorm2d(64)
        )
        # Flatten layer
...
class Decoder(nn.Module):
    ...
    def __init__(self, encoded_space_dim):
        ...
        self.decoder_conv = nn.Sequential(
            nn.ConvTranspose2d(in_channels=64, out_channels=32, kernel_size=5, stride=3, padding=2), # 4 x 4
            nn.ReLU(True),
            nn.BatchNorm2d(32),
            nn.ConvTranspose2d(in_channels=32, out_channels=16, kernel_size=5, stride=3, padding=3), # 8 x 8
            nn.ReLU(True),
            nn.BatchNorm2d(16),
            nn.ConvTranspose2d(in_channels=16, out_channels=3, kernel_size=3, stride=1, padding=0), # 10 x 10
        )
